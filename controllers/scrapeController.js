 ...
 */
async function saveScrapedPage(hotelUuid, url, html, htmlOrigin, markdown, checksum, depth, pageId = null) {
  // Validate inputs
  ...
  try {
    if (targetId) {
      ...
      const updateQuery = `
        ...
        WHERE id = ?
      `;
      const result = await executeQuery(updateQuery, [html, htmlOrigin, markdown, checksum, depth, targetId]);
        ...
        const bodyHtml = await page.evaluate((currentDepth) => {
          ...
          if (currentDepth > 0) {
            document.querySelectorAll(
              'nav, header, footer, #header, #footer, #nav, .header, .footer, [role="header"], [role="footer"], .navbar, .navigation, .site-nav, .breadcrumb, #navbar, #navigation, #site-nav, #breadcrumb, [role="navigation"], [role="banner"]'
